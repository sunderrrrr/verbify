...
4. Если текст обрывается на одной странице и продолжается на следующей - аккуратно соедини, не искажая содержания
5. Верни ЕДИНЫЙ связный текст, не изменяя ничего в нем.
6. ВНОСИТЬ КОММЕНТАРИИ И ИНЫЕ ПРАВКИ ЗАПРЕЩЕНО - ТЫ ДОЛЖЕН ВЕРНУТЬ ТОЛЬКО ИСХОДНЫЙ ТЕКСТ.
